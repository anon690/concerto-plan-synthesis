...
from concerto_scheduling_examples.component_types import centraluser_assembly
...
if __name__ == "__main__":
    ...
    a = centraluser_assembly(n)
    initially_active_places = {c: {c.type().get_place("running")} for c in a.instances()}
    goal_behaviors = {c: {c.type().get_behavior("update")} for c in a.instances() if c.id() != "u"}
    goal_state = {}
